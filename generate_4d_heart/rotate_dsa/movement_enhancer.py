...
class MovementEnhancer(Protocol):
    def __init__(self, cavity_label: torch.Tensor, coronary_label: torch.Tensor, device: torch.device) -> None:
        ...
...
class CoronaryBoundLV(MovementEnhancer):
    def __init__(
        ...
        with cp.cuda.Device(device.index):
            ...
            lv_cp = cp.from_dlpack(tensor2dlpack(lv.to(device))).astype(cp.bool_)
            myo_cp = cp.from_dlpack(tensor2dlpack(myo.to(device))).astype(cp.bool_)
            
            coronary_cp = cp.from_dlpack(tensor2dlpack(coronary_label.squeeze().to(device))).astype(cp.uint8)
