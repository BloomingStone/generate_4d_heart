...
logger = logging.getLogger("batch-gen-cta-imageCAS")
...
output_root = Path("/media/data3/sj/Data/imageCAS/gen_4d_output/cta")
...
for sub_dvf_dir, roi_json, image_nii, coronary_nii in zip(
    ...
    try:
        ...
        logger.exception(f"{dvf_dir.name} failed due to error: {e}")
        continue
